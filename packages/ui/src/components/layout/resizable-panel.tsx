    ...
    ({ children, minWidth = 200, maxWidth = 600, className, ...props }, ref) => {
        ...
        return (
            <div className="flex h-full">
                <motion.div
                    ref={ref}
                    className={cn("relative overflow-hidden border-r border-border bg-surface", className)}
                    style={{ width }}
                    {...props}
                >
                    {children}
                    <motion.div
                        className="absolute right-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-brand active:bg-brand"
                        drag="x"
